...
class ZillowScraper:
    ...
    def run(self):
        for page in range(1, 5):
            ...
            res = self.fetch(
                'https://www.zillow.com/pittsburgh-pa/rentals/{}'.format(str(page))+'_p/?searchQueryState=%7B%22pagination%22%3A%7B%7D%2C%22usersSearchTerm%22%3A%22Pittsburgh%2C%20PA%22%2C%22mapBounds%22%3A%7B%22west%22%3A-80.30758186767581%2C%22east%22%3A-79.61750313232425%2C%22south%22%3A40.25792721551524%2C%22north%22%3A40.63468195850491%7D%2C%22regionSelection%22%3A%5B%7B%22regionId%22%3A26529%2C%22regionType%22%3A6%7D%5D%2C%22isMapVisible%22%3Atrue%2C%22filterState%22%3A%7B%22fsba%22%3A%7B%22value%22%3Afalse%7D%2C%22nc%22%3A%7B%22value%22%3Afalse%7D%2C%22fore%22%3A%7B%22value%22%3Afalse%7D%2C%22cmsn%22%3A%7B%22value%22%3Afalse%7D%2C%22auc%22%3A%7B%22value%22%3Afalse%7D%2C%22pmf%22%3A%7B%22value%22%3Afalse%7D%2C%22pf%22%3A%7B%22value%22%3Afalse%7D%2C%22fr%22%3A%7B%22value%22%3Atrue%7D%2C%22fsbo%22%3A%7B%22value%22%3Afalse%7D%7D%2C%22isListVisible%22%3Atrue%2C%22mapZoom%22%3A11%7D',
                params)
            self.parse(res.text)
...
if __name__ == '__main__':
    ...
    df_normal = df[df['price'].notna()]
    df_normal = df_normal.applymap(str)

    df_split = df_normal['details'].str.split("', '", expand=True)
    df_split.columns = ['bds', 'ba', 'sqft']
    df_split['bds'] = df_split['bds'].map(lambda x: re.sub('\D', '', x))
    df_split['ba'] = df_split['ba'].map(lambda x: x[:-3])
    df_split['sqft'] = df_split['sqft'].map(lambda x: re.sub('\D', '', x))

    df_normal = pd.concat([df_normal, df_split], axis=1)
    df_normal.drop(['details'], axis=1, inplace=True)

    df_normal['zipcode'] = df_normal['address'].map(lambda x: x[-8:])
    df_normal.to_csv(r'zillow_rent2.csv', index=None)
